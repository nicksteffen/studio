    ...
    <main className="w-full mx-auto py-12 px-4 sm:px-6 lg:px-8">
      <div className="text-center mb-8">
        ...
        <p className="mt-2 text-lg text-foreground/80 max-w-2xl mx-auto">
          Get inspired by what others are dreaming of. Add ideas from their
          lists to your own with a single click.
        </p>
      </div>
      ...
      {!error && communityLists.length === 0 && !searchTerm ? (
        <div className="text-center py-12">
          <p className="text-muted-foreground">
            No public lists found yet. Be the first to make your list public!
          </p>
        </div>
      ) : null}
      ...
      {!error && communityLists.length > 0 ? (
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {communityLists.map((list: CommunityList) => {
            ...
            const userAvatar =
              list.profiles?.avatar_url ?? `https://placehold.co/100x100.png`;
            ...
            return (
              <Card
                key={list.id}
                className="shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col min-w-0"
              >
                <CardHeader>
                  <Link
                    href={publicProfileUrl}
                    className="flex items-center gap-4 group hover:no-underline"
                  >
                    <Avatar>
                      <AvatarImage
                        src={userAvatar}
                    ...
                    <div>
                      <CardTitle className="font-headline text-lg group-hover:underline">
                        {list.title}
                      </CardTitle>
                    ...
                    <ul className="space-y-2 text-sm">
                      ...
                      {list.list_items.length > 4 && (
                        <li className="text-xs text-muted-foreground pt-1">
                          ...and {list.list_items.length - 4} more.
                        </li>
                      )}
                    </ul>
                  ) : (
