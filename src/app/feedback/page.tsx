...
export default async function FeedbackPage() {
  ...
  if (!user) {
    console.log("must be logged in, show toast ")
  ...
  if (suggestionsError) {
    ...
    return <div className="flex flex-col items-center justify-center">Error loading suggestions.</div>;
  }
  ...
  const suggestionsWithVotes: Suggestion[] = (suggestionsData || []).map(suggestion => {
    const suggestionVotes = allVotes.filter(vote => vote.suggestion_id === suggestion.id);
    const upvotes = suggestionVotes.filter(vote => vote.vote_type === 'upvote').length;
    const downvotes = suggestionVotes.filter(vote => vote.vote_type === 'downvote').length;
