...
const GoogleSignInButton = () => {
    ...
    async (response: CredentialResponse) => {
      try {
        if (!response.credential) {
          console.error(
            "No credential found in Google Sign-In button response.",
          );
          return;
        }
        ...
        if (!storedNonce) {
          console.error(
            "CRITICAL: Nonce not found in session storage for button sign-in. This will cause a mismatch.",
          );
        ...
        if (error) {
          ...
          if (error.message.includes("nonce mismatch")) {
            console.error(
              "SPECIFIC ERROR: Nonce mismatch. The nonce sent to Supabase did not match the one from Google.",
            );
          }
          throw error;
        }
        ...
        console.error(
          "Failed to handle Google Sign-In button credential response:",
          error,
        );
  ...
  const initializeGoogleButton = useCallback(async () => {
    ...
    console.log(
      "Google GSI script loaded. Initializing Google Sign-In button...",
    );
    ...
    console.log(
      ...
      sessionStorage.getItem(NONCE_STORAGE_KEY),
    ); // Verify storage
    ...
    if (
      ...
      if (!googleClientId) {
        console.error(
          "CRITICAL: NEXT_PUBLIC_GOOGLE_CLIENT_ID is undefined or empty for Google Sign-In button!",
        );
        return;
      }
      ...
      console.error(
        "Google Identity Services library not fully loaded for button initialization.",
      );
    }
  ...
  return (
    <>
      ...
      <Script
        ...
        onError={(e) =>
          console.error("Error loading Google GSI client script for button:", e)
        }
